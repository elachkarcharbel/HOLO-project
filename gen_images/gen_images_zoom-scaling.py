...
import cv2
# --- PARAMETERS ---
OUTPUT_DIR = "synthetic-images-zc"
LABELS_DIR = "labels"
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(LABELS_DIR, exist_ok=True)

CSV_PATH = os.path.join(LABELS_DIR, "z_labels_zc.csv")
...
def generate_scene_zoom(scene_id, csv_file, counter):
    amp = gedanken_pattern(size=params['patch_size'])
    wave = AxialPlane(amp, 0, params)

    for z in Z_LEVELS:
        re, im = wave(z)
        intensity = np.sqrt(re**2 + im**2).astype('float32')

        # --- Synthetic zoom scaling ---
        zoom_factor = 1 + (z / max(Z_LEVELS)) * 0.5  # up to +50% zoom
...
def main():
    ...
    with open(CSV_PATH, "w") as csv_file:
        ...
        for scene_id in tqdm(range(total_scenes), desc="Generating synthetic holograms with zoom scaling"):
            generate_scene_zoom(scene_id, csv_file, counter)
