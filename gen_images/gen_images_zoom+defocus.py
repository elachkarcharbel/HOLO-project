...
import cv2
# --- PARAMETERS ---
OUTPUT_DIR = "synthetic-images-pd_zc"
LABELS_DIR = "labels"
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(LABELS_DIR, exist_ok=True)

CSV_PATH = os.path.join(LABELS_DIR, "z_labels_pd_zc.csv")
...
def generate_scene_combined(scene_id, csv_file, counter):
    amp = gedanken_pattern(size=params['patch_size'])

    for z in Z_LEVELS:
        # --- Add phase curvature (defocus) ---
        amp_z = apply_focus(amp, z, params)
        wave = AxialPlane(amp_z, 0, params)
        re, im = wave(z)
        intensity = np.sqrt(re**2 + im**2).astype('float32')

        # --- Add synthetic zoom scaling ---
        zoom_factor = 1 + (z / max(Z_LEVELS)) * 0.5
        new_size = int(params['patch_size'] * zoom_factor)
        zoomed = cv2.resize(intensity, (new_size, new_size), interpolation=cv2.INTER_LINEAR)
        c = new_size // 2
        half = params['patch_size'] // 2
        zoomed = zoomed[c - half:c + half, c - half:c + half]

        z_str = f"{int(z):03d}"
        img_name = f"scene{scene_id:05d}_z{z_str}um.png"
        plt.imsave(os.path.join(OUTPUT_DIR, img_name), zoomed, cmap='gray')
...
def main():
    ...
    with open(CSV_PATH, "w") as csv_file:
        ...
        for scene_id in tqdm(range(total_scenes), desc="Generating synthetic holograms with zoom and phase defocus combined"):
            generate_scene_combined(scene_id, csv_file, counter)
